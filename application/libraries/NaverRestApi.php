...
//		echo "sign = " . $sign . "\n";
	...
	protected function getHeader($method, $uri){
		...
		$header = array('Content-Type: application/json; charset=UTF-8', 'X-Timestamp: ' . $timestamp, 'X-API-KEY: ' . $this->apiKey, 'X-Customer: ' . $this->customerId, 'X-Signature: ' . $this->generateSignature($timestamp, $method, $uri),);
		return $header;
	}
